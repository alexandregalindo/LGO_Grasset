...
const DepartmentList = () => {
    ...
    const addDepartment = (data) => {
        axios.post('http://localhost:4081/api/departments', data)
            .then(response => {
    ...
    const deleteDepartment = (departmentId) => {
        axios.delete(`http://localhost:4081/api/departments/${departmentId}`)
            .then(response => {
            ...
            .finally(() => {
                // Fermez la boîte de dialogue, que la suppression ait réussi ou échoué.
                setDisplayDeleteDialog(false);
            });
    }

    const editDepartment = (data) => {
        axios.put(`http://localhost:4081/api/departments/${editingDepartment.id}`, data)
            .then(response => {
                DepartmentService.findAll().then(data => setDepartments(data));
            })
            .catch(error => {
